# Andrii Khorsun · Full-Stack Engineer

**Full-Stack Engineer at [Cruxlab](https://cruxlab.com)**  
Working across the full engineering lifecycle — development, testing, and production systems.

---

**Building in spare time:**  
**[Shared Tales](https://shared-t.online/)** — collaborative book-writing platform powered by a 4-node LangGraph multi-agent system (planner → writer → editor → summarizer). 
Next.js · Express · PostgreSQL · Anthropic API

*[frontend](https://github.com/AndrewKhorsun/shared-tails-app-next)*


---

**Currently deepening:** DevOps — networking, reverse proxy, load balancing, containers  
**Target:** Senior Full-Stack / AI-focused roles  

---

**Stack**

`TypeScript` `React` `Next.js` `SvelteKit` `Node.js` `Express` `PostgreSQL`  
`LangGraph` `Anthropic API` `Docker` `GitHub Actions` `nginx` `Linux`

---

[LinkedIn](https://linkedin.com/in/andrii-khorsun) · [andrewkhorsun@gmail.com](mailto:andrewkhorsun@gmail.com) · [Telegram](https://t.me/AndrewKhorsun)
